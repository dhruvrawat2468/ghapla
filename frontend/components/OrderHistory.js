...
const OrderHistory = () => {
  ...
  const [orders, setOrders] = useState({ completed: [], ongoing: [], declined: [] });
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Fetch orders for the current technician
      const response = await Promise.race([
        fetch(`http://192.168.1.13:7000/api/orders/technician/${technician.id}`, {
          headers: {
      ...
      const categorizedOrders = {
        ...
        ongoing: data.filter(order => order.status === 'ongoing' || order.status === 'assigned'),
        declined: data.filter(order => order.status === 'declined' || order.status === 'missed')
      };
      ...
      // If we have no real data, use some placeholder data for testing
      if (technician.id === "tech123") {
        setOrders({
          completed: [
            {
              id: "1",
